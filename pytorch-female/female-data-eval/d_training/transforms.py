...
class ImageClassificationWithBBoxes(nn.Module):
    def __init__(
        self,
        crop_size: int,
        resize_size: int = 256,
    ...
    def forward(self, img: Tensor, target: dict) -> Tuple[Tensor, dict]:
        if not isinstance(img, Tensor):
            img = F.pil_to_tensor(img)
            img = F.convert_image_dtype(img, torch.float)
            
        orig_size = img.size()[-2:]  # original height and width
        img = F.resize(img, self.resize_size, interpolation=self.interpolation, antialias=self.antialias)
        img = F.center_crop(img, self.crop_size)
        new_size = img.size()[-2:]  # new height and width
        ...
        boxes = target["boxes"]
        new_boxes = self.resize_boxes(boxes, orig_size, new_size)
        target["boxes"] = new_boxes
        
        return img, target

    def resize_boxes(self, boxes, orig_size, new_size):
        ratios = [n / o for n, o in zip(new_size, orig_size)]
        ratio_height, ratio_width = ratios
        new_boxes = boxes.clone()
        new_boxes[:, [0, 2]] *= ratio_width
        new_boxes[:, [1, 3]] *= ratio_height
        return new_boxes
    ...
    def describe(self) -> str:
        return (
            ...
            f"``[0.0, 1.0]`` and then normalized using ``mean={self.mean}`` and ``std={self.std}``. Bounding boxes are resized accordingly."
        )

class ImageClassificationWithBBoxes2(nn.Module):
    def __init__(
        self,
        crop_size: int,
        resize_size: int = 256,
    ...
    def forward(self, img: Tensor, target: dict) -> Tuple[Tensor, dict]:
        ...
        # Remap class labels if class_mapping is provided
        if self.class_mapping:
            labels = target["labels"]
            new_labels = self.remap_labels(labels)
    ...
    def remap_labels(self, labels):
        return torch.tensor([self.class_mapping[label.item()] for label in labels], dtype=torch.int64)
    ...
    def describe(self) -> str:
        return (
            ...
            f"followed by a central crop of ``crop_size={self.crop_size}``. Finally the values are first rescaled to "
            f"``[0.0, 1.0]`` and then normalized using ``mean={self.mean}`` and ``std={self.std}``. Bounding boxes are resized accordingly."
            "If a ``class_mapping`` dictionary is provided, the class labels will be remapped accordingly."
        )

# MaskRCNN-specific code below

class ImageClassificationWithBBoxesSegments(nn.Module):
    def __init__(
        self,
        crop_size: int,
        resize_size: int = 256,
    ...
    def forward(self, img: Tensor, target: dict) -> Tuple[Tensor, dict]:
        ...
        if "masks" in target:
            masks = target["masks"]
            new_masks = self.resize_masks(masks, orig_size, new_size)
    ...
    def resize_masks(self, masks, orig_size, new_size):
        resized_masks = F.resize(masks, new_size, interpolation=InterpolationMode.NEAREST)
    ...
    def describe(self) -> str:
        return (
            ...
            f"The images are resized to ``resize_size={self.resize_size}`` using ``interpolation={self.interpolation}``, "
            f"followed by a central crop of ``crop_size={self.crop_size}``. Finally the values are first rescaled to "
            f"``[0.0, 1.0]`` and then normalized using ``mean={self.mean}`` and ``std={self.std}``. Bounding boxes and masks are resized accordingly. "
            "If a ``class_mapping`` dictionary is provided, the class labels will be remapped accordingly."
        )
