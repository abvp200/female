...
class EvaluationLightningModule(LightningModule):
    ...
    def evaluate_epoch_end(self, stage, results):
        ...
        self.log_metrics(stage, coco_eval.stats)

    def log_metrics(self, stage, stats):
        self.log(f"{stage}_mAP", stats[0], sync_dist=True)
        self.log(f"{stage}_mAP_50", stats[1], sync_dist=True)
        self.log(f"{stage}_mAP_75", stats[2], sync_dist=True)
        self.log(f"{stage}_mAP_small", stats[3], sync_dist=True)
        self.log(f"{stage}_mAP_medium", stats[4], sync_dist=True)
        self.log(f"{stage}_mAP_large", stats[5], sync_dist=True)
        self.log(f"{stage}_AR_1", stats[6], sync_dist=True)
        self.log(f"{stage}_AR_10", stats[7], sync_dist=True)
        self.log(f"{stage}_AR_100", stats[8], sync_dist=True)
        self.log(f"{stage}_AR_small", stats[9], sync_dist=True)
        self.log(f"{stage}_AR_medium", stats[10], sync_dist=True)
        self.log(f"{stage}_AR_large", stats[11], sync_dist=True)
...
def main():
    ...
    for repeat_id in [0, 1, 2, 3, 4]:
        ...
        mlflow_logger = MLFlowLogger(
            experiment_name='FasterRCNN_Evaluation_v3_nonStrat_allClasses_v16',
