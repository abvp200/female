...
import json
from transforms import ImageClassificationWithBBoxes2  #, ImageClassificationWithBBoxesSegments
...
def get_transforms():
    return ImageClassificationWithBBoxes2(
        crop_size=640, 
        resize_size=680,
        class_mapping={  # MaskRCNN expects class_ids to start with 1. Merge classes 4 & 5. 
            0:1,
            1:2,
            2:3,
            3:4,
...
if __name__ == "__main__":
    ...
    for i in [3,4]:  #0,1,2,
        # Main script
        data_dir = f'/mnt/data_vilen/05_model_input/repeated_nonstratified_train_val_test_split/yolo_v6_all_classes_COCO_v4_noEmpty/repeated_split_{i}'
        num_classes = 9
        split_type = 'nonStratSplit'

        # Configure MLFlow logger
        mlflow_logger = MLFlowLogger(
            experiment_name=f'FasterRCNN_ALL_CLS_DSet_v3__{split_type}_classesNo{num_classes}',
        ...
        model_name = f'resnet101_FINALv4_repeat{i}'
        weights ='ResNet101_Weights.IMAGENET1K_V1'
        
        checkpoint_name = f"{model_name}_{weights.replace('.','')}_{class_ids_str}_v4_10ep__no_augm_{split_type}_repeat{i}"
