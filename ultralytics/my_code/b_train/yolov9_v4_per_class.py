...
def freeze_all(layer_name: str) -> bool:
    return freeze(layer_name, 1, 41)  # freeze all layers from 1 to 42 (not 43)
...
for class_id in [0,1,2,3,4,5,6,7,8]:
    for freezing_strategy in [freeze_mid]:
        print('='*80)
        for split_no in [0,1,2,3,4]:
            ...
            # Train the model
            model.train(data=f"/mnt/data_vilen/05_model_input/repeated_stratified_train_val_test_split/ultralitics_per_class_{class_id}_COCO_v4_noEmpty/repeated_split_{split_no}/data.yml", 
                        epochs=8, 
                        imgsz=640,
                        batch=16,
                        device="0,1")