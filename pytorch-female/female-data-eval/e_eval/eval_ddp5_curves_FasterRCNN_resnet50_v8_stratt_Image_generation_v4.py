...
def main():
    ...
    for idx in selected:
        ...
        # Resize for model input and overlays (PIL expects (width, height))
        resized = original.resize((1920, 1080))

        # Ground truth: use raw COCO annotations scaled to resized image
        ...
        if raw_boxes:
            ...
            labels_tensor = torch.tensor(raw_labels, dtype=torch.int64)
            # scale factors
            scale_w = 1920.0 / original.width
            scale_h = 1080.0 / original.height
